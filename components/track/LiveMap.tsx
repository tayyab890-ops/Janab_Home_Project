...
const riderIcon = L.divIcon({
    html: `<div class="bg-primary p-2 rounded-full border-2 border-white shadow-lg animate-bounce animate-pulse">
...
export default function LiveMap({ order }: LiveMapProps) {
    ...
    return (
        <div className="h-full w-full rounded-[2rem] overflow-hidden z-0 shadow-inner bg-slate-100">
            <MapContainer
                center={rider}
                zoom={14}
            ...
            >
                ...
                <Marker position={pickup} icon={icon}>
                    <Popup>
                        ...
                        <div className="text-xs">{order?.pickupAddress || order?.origin?.name || "Pickup location"}</div>
                    </Popup>
                </Marker>

                <Marker position={dropoff} icon={icon}>
                    <Popup>
                        <div className="font-bold">Destination</div>
                        <div className="text-xs">{order?.deliveryAddress || order?.destination?.name || "Delivery destination"}</div>
                    </Popup>
                </Marker>
