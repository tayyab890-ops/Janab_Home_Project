...
import { Package, MapPin, Scale, DollarSign, Send } from "lucide-react";
    ...
    return (
        <div className="space-y-8 pb-10">
            <div className="flex justify-between items-end">
                <div>
                    <h1 className="text-3xl font-bold text-slate-800">Hello, {user?.name || "User"}! 👋</h1>
                    <p className="text-slate-500">Need something delivered today?</p>
                </div>
                ...
                <div className="md:col-span-2">
                    ...
                    >
                        <div className="flex items-center space-x-3 mb-8">
                            ...
                            <h2 className="text-xl font-bold">Book a New Delivery</h2>
                        </div>
                ...
                <div className="space-y-8">
                    <div className="bg-slate-900 text-white p-8 rounded-[2rem] relative overflow-hidden">
                        <div className="relative z-10">
                            <p className="opacity-70 text-sm">Total Deliveries</p>
                    ...
                    <div className="bg-white p-8 rounded-[2rem] shadow-xl border border-slate-100 flex-1">
                        ...
                        <div className="space-y-4">
                            {orders.length === 0 ? (
                                ...
                                <div key={order.id} className="flex items-center space-x-4 border-b border-slate-50 pb-4 last:border-0 last:pb-0">
                                    <div className="w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center text-primary">
                                        <Package size={20} />
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <p className="text-sm font-bold truncate">#{order.trackingId}</p>
                                        <p className="text-xs text-slate-500 truncate">Status: {order.status}</p>
                                    </div>
                                    <div className="text-right">
                                        <span className={`text-[10px] px-2 py-1 rounded-full font-bold uppercase ${order.status === 'DELIVERED' ? 'bg-green-100 text-green-600' : 'bg-blue-100 text-blue-600'
                                            }`}>
                                            {order.status}
                                        </span>
                                    </div>
